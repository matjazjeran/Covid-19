...
# changes on 06.04.: added last day to simulation graph, added Serbia
# changes on 09.04.: Fatalities subtracted from infected:  Infected <- Infected - Recovered - Dead
# changes on 10.04.: computation for New Zealand added
# changes on 18.04.: automatic plot to files added dependeing on parameter image.to.file
...
# image.to.file <- FALSE  # image to file (TRUE) or to screen (FALSE)
image.to.file <- TRUE  # image to file (TRUE) or to screen (FALSE)
...
# Country <- 'SRB'  # Serbia - 10.04: unstable simulation depending on N.int
# Country <- 'BEL'  # Belgium - 05.04.: fits data, peak not yet confirmed
# Country <- 'DEU'  # Germany - 03.04.: fits data, 08.04.: peak confirmed
# Country <- 'ESP'  # Spain - 03.04.: fits data, peak not yet confirmed, daily increase of infections has dropped down
# Country <- 'GBR'  # UK - 06.04.: fits data, long epidemy ahead? 10.04.: fits data, near the peak?
...
# Country <- 'USA'  # USA - 09.04.: fits data, 
...
if (Country == "CHN") {
 StartDate <- as.Date ("2020-01-31")  # Starting day when first infected was counted > 0
...
if (Country == "DNK") {
 StartDate <- as.Date ("2020-02-29")  # Starting day when first infected was counted > 0
 N.int1 <- 14 # starting day of intervention
 NSim <- 100  # No of days for simulation
...
if (Country == "HUN") {
 StartDate <- as.Date ("2020-03-05")  # Starting day when first infected was counted > 0
 N.int1 <- 19 # starting day of intervention
 NSim <- 100  # No of days for simulation
...
if (Country == "FRA") {
 StartDate <- as.Date ("2020-01-27")  # Starting day when first infected was counted > 0
 N.int1 <- 40 # starting day of intervention
 NSim <- 100  # No of days for simulation
...
if (Country == "AUT") {
 StartDate <- as.Date ("2020-02-25")  # Starting day when first infected was counted > 0
 N.int1 <- 21 # starting day of intervention
...
if (Country == "HRV") {
 StartDate <- as.Date ("2020-02-25")  # Starting day when first infected was counted > 0
 N.int1 <- 26 # starting day of intervention
 NSim <- 80  # No of days for simulation
...
if (Country == "SRB") {
 StartDate <- as.Date ("2020-03-09")  # Starting day when first infected was counted > 0
...
if (Country == "BEL") {
 StartDate <- as.Date ("2020-03-01")  # Starting day when first infected was counted > 0
...
if (Country == "ESP") {
 StartDate <- as.Date ("2020-03-05")  # Starting date selected by trial and error
 N.int1 <- 23 # starting day of intervention
 NSim <- 80  # No of days for simulation
...
if (Country == "GBR") {
 StartDate <- as.Date ("2020-02-29")  # Starting date selected by trial and error
 N.int1 <- 23 # starting day of intervention - none intervention really so far - so last day
 NSim <- 80  # No of days for simulation
...
if (Country == "CHE") {
 StartDate <- as.Date ("2020-03-01")  # Starting day set manually
 N.int1 <- 16 # starting day of intervention set manually
 NSim <- 80  # No of days for simulation
...
if (Country == "SWE") {
 StartDate <- as.Date ("2020-02-25")  # Starting day set manually
 N.int1 <- 22 # starting day of intervention set manually
 NSim <- 120  # No of days for simulation
...
if (Country == "SGP") {
 StartDate <- as.Date ("2020-01-23")  # Starting day set manually
 N.int1 <- 49 # starting day of intervention set manually
 NSim <- 180  # No of days for simulation
...
if (Country == "KOR") {
 StartDate <- as.Date ("2020-01-22")  # Starting day when first infected was counted > 0
 N.int1 <- 38 # starting day of intervention set manually  reasonably good fit for 29,  but awfull for 28 and 30
 NSim <- 100  # No of days for simulation
# Singapore is a difficult case that deserves special treatment as there were two vawes of epidemy
# parameters describing medical care - very dependent on the medical resources - so far the same as in original code
ShareSevereCase    <- 0.2   # share of severe cases - sent to hospital
...
if (Country == "USA") {
 StartDate <- as.Date ("2020-03-01")  # Starting day set manually
 N.int1 <- 29 # starting day of intervention - not really started
 NSim <- 100  # No of days for simulation
...
tmp.fname <- paste0 ("Ozdraveli_", Country, "_", format (Sys.Date (), "%Y%m%d"), ".png")
if (image.to.file) png (filename = tmp.fname) else dev.new ()
plot (Day, Recovered, type ="b", xlab = "Day", ylab = 'Recovered (linear scale)', main = paste ("Total recovereds COVID-19", Country))
if (image.to.file) dev.off ()

# plot daily increase of infected
tmp.fname <- paste0 ("Dnevne_inf_", Country, "_", format (Sys.Date (), "%Y%m%d"), ".png")
if (image.to.file) png (filename = tmp.fname) else dev.new ()
...
tmp.fname <- paste0 ("Sim_", Country, "_", N.int1, "_", format (Sys.Date (), "%Y%m%d"), ".png")
if (image.to.file) png (filename = tmp.fname) else dev.new ()
...
title (paste ("Total infections of COVID-19", Country, " - whole simulation model"),
 sub = paste ("StartDate = ", StartDate, "  N.int =", N.int1, "  Last day = ", length(Infected), " : ", StartDate+length(Infected)))
...
col = c ("blue", "orange", "brown", "black", "red"), pch = 1, bg = "transparent")
legend ("topright", legend = c (paste ("R0_1 =", round (par1$R0, digit = 3)), paste ("R0_2 =", round (par2A$R0, digit = 3))), 
bg = "transparent")
if (image.to.file) dev.off ()
